...
class PlainFormService extends BaseApplicationComponent
{
    ...
    public function sendEmailNotification($form, $message, $postData, $html = true, $email = null)
    {
        // Generic errors bool
        $errors = false;
        ...
        $email->fromEmail = craft()->templates->renderObjectTemplate($form->fromEmail, $postData);
        $email->replyTo   = craft()->templates->renderObjectTemplate($form->replyToEmail, $postData);
        $email->sender    = craft()->templates->renderObjectTemplate($form->fromEmail, $postData);
        $email->fromName  = craft()->templates->renderObjectTemplate($form->fromName, $postData);
        $email->subject   = craft()->templates->renderObjectTemplate($form->emailSubject, $postData);
